...
function printHelp ()
{
    ...
    echo -e "${GREEN}-n ${RESET}defaults to ${GREEN}hyperledger-eth-network${RESET}. use ${YELLOW}-n your-named-network ${RESET}if you are using a different network name"  | indent
...
{
    echo ""
    echo -e "${YELLOW}network archive, start and deploy script for the Hyperledger Fabric Eth Network" | indent
...
NETWORK_NAME="hyperledger-eth-network"
